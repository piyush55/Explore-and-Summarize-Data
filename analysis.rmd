...
factor_distribution <- function(df, variable, binwidth= 1, transform='') {
  ...
  print(cowplot::plot_grid(hist, box, rel_heights=c(0.65, 0.35),
                           ncol = 1, align = "v"))
...
plot_top_freqs <- function(freqs, variable, fill, n=50) {
  # calculate the top n frequencies from a
  #variable and plot as a horizontal bar plot
...
paste('The number of special transactions in the ddataset are : ',
      totTransactionsNo)
```
...
cowplot::plot_grid(hist1, box1, rel_heights=c(0.65, 0.35), ncol = 1,
                   align = "v")
```
...
hist1 <- ggplot(data=df, aes(x=UnitPrice)) +
      geom_histogram(binwidth = 10) +
      # xlim(0, 1000) +
      labs(title='UnitPrice Distribution', x = 'UnitPrice(GBP)')

hist2 <- ggplot(data=df, aes(x=UnitPrice)) +
      geom_histogram(binwidth = 10) +
      # xlim(0, 1000) +
      scale_x_log10()+
      labs(title='log10 UnitPrice Distribution', x = 'UnitPrice(GBP)')
...
country_top_freqs <- plot_top_freqs(freqs = country_freqs, 
                                    variable = 'Country', fill = 'mediumpurple', 37)
...
p2 <- aggregate(cbind(revenue = Value) ~ Date, 
          ...
          geom_line() +
          guides(colour=FALSE) +
          labs(x = "Invoice Date", y = 'Revenue(GBP)') + 
...
daily_rev %>%
  ggplot() +
    ...
    labs(x = "Invoice Date", y = 'Revenue(GBP)', title = 'Value over Time') 
```
...
ggplot(aes(x = Date, y = revenue, group = 1), data = monthly_rev) +
  geom_line() +
    ...
    scale_x_date(breaks = date_breaks("months"), date_labels = "%b-%y") +
    theme(axis.text.x = element_text(angle = 45)) + 
    labs(x = "Invoice Month", y = 'Revenue(GBP)') + 
...
p2 <- df %>%
        ...
        summarize(revenue = sum(Value)) %>%
          ...
          geom_col(fill = "lightseagreen") + 
          scale_x_continuous(breaks=c(2010, 2011)) +
          labs(x = '', y = 'Revenue(GBP)', title = 'Revenue per year')
...
ggplot(aes(x = InvoiceHour, y = UnitPrice), data = dfs) + 
  ...
  geom_boxplot(aes(group = dfs$InvoiceHour)) + 
  labs(y = 'UnitPrice(GBP)')
```
...
ggplot(aes(x = factor(InvoiceWday), y = UnitPrice,
  ...
  geom_boxplot(aes(group = InvoiceWday)) + 
...
country_price %>%
   ...
   coord_flip() +
   theme_bw(base_size = 7)  +
   labs(title="", y ="Avg Unit Price(GBP)", x = '')
...
country_rev_price <- dfs %>%
  ...
  labs(x = 'Avg UnitPrice(GBP)', y = 'Sum of Revenue(GBP)')
```
...
country_avgrev_avgprice <- dfs %>%
  ...
  geom_point() +
  geom_smooth(method = 'lm', color = 'red') + 
  labs(x = 'Avg UnitPrice(GBP', y = 'Avg Revenue(GBP)')
```
...
country_avgrev_avgprice %>%
   ...
   labs(title="", y ="Avg Revenue(GBP)", x = '')
```
...
ggplot(aes(x = InvoiceHour, y = UnitPrice, colour = factor(Country),
           legend = F),
       data = dfs) + 
  geom_point(alpha = 1/20) +
  facet_wrap(~Country) +
...
```{r, echo=FALSE, fig.height=7, fig.width=8}
ggplot(aes(x = InvoiceWday, y = UnitPrice, colour = factor(Country),
           legend = F),
       data = dfs) + 
  geom_point(alpha = 1/20) +
  facet_wrap(~Country) +
...
mycolors = c(brewer.pal(name="Paired", n = 12),
...
country_rev_price %>%
    ggplot(aes(x = avg_unit_price, y = log10(sum_revenue), colour = factor(Country), 
      ...
      labs(title = 'Country Quadrant plot of log10(Sum of Value) vs the Avg Unit Price',
           x= 'Avg Unit Price(GBP)',
          y= 'log10(Sum of Revenue(GBP))')
```
...
customer_country_sumrev %>% 
   ...
   labs(title="", y ="Sum of Revenue(GBP)", x = 'Customer ID')
```
...
# Plot a world map - Using logFixedWidth to get some resolution for the rest of the countries
par(mai=c(0,0,0.2,0), xaxs="i",yaxs="i")
mapCountryData(colourPalette = 'topo', missingCountryCol = 'grey', catMethod ='logFixedWidth',  mapped_data, nameColumnToPlot = "revenue",  mapTitle = "Global Map per Revenue(GBP)")
...
country_rev %>%
  ggplot(aes(x = reorder(Country, revenue), y = revenue)) + 
   ...
   coord_flip() +
   theme_bw(base_size = 7)  +
   labs(title="Country bar-plot per Revenue", y ="Revenue(GBP)", x = 'Country')
```
...
country_rev_price %>%
    ggplot(aes(x = avg_unit_price, y = log10(sum_revenue),
      ...
      geom_vline(xintercept = 3) +
          geom_hline(yintercept = mean(log10(country_rev_price$sum_revenue)))+
      theme(legend.position="bottom") +
      labs(title =
             'Country Quadrant plot of log10(Sum of Value) vs the Avg Unit Price',
           x= 'Avg Unit Price(GBP)',
           y= 'log10(Sum of Revenue(GBP))')
```
